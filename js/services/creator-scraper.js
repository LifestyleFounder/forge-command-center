...
//                         content_structure, topic, topic_summary, call_to_action, is_analyzed, analyzed_at
...
export async function getAllPosts(limit = 500) {
  ...
  try {
    ...
    console.warn('[creator-scraper] getAllPosts failed', err);
    return [];
  }
}
...
// ── Simplified data source — Supabase only ──────────────────────────

export async function getCreatorsWithFallback() {
  const sbCreators = await getCreators();
  const sbPosts = sbCreators.length > 0 ? await getAllPosts(500) : [];

  if (sbCreators.length > 0) {
